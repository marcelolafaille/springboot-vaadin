...
COPY --chown=usuario:usuario frontend frontend
COPY --chown=usuario:usuario package.json ./

# Using * after the files that are autogenerated so that so build won't fail if they are not yet created
COPY --chown=usuario:usuario package-lock.json* pnpm-lock.yaml* webpack.config.js* ./
